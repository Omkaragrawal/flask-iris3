...
def index():
    if request.method == "POST":
      if request.form: return(render_template('index.html', results = prediction(request)))
      else:  return jsonify(results = prediction(request))
    else: return render_template('index.html', results = None)
